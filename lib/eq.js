...
export function getEntries(eventId) {
  ...
  entries.forEach((en) => {
    ...
    en._s = (en.horse + " " + en.rider + " " + en.owner).toLowerCase();
  });
...
/* Genuinely-live fixtures for a country: "today" — anchored to the newest
   event in the catalogue so the demo always reads as current — sits within
   `pad` days either side of the event, i.e. from the eve of day one to the day
   after the last. Newest-first; empty when the country has nothing live, so
   the caller can drop the strip entirely rather than fall back to the archive. */
export function liveEventsForCountry(code, pad = 1) {
  const DAY = 86400000;
  const ends = (e) => e.dateEnd || e.date;
  const now = Math.max.apply(null, EVENTS.map((e) => +new Date(ends(e) + "T12:00:00")));
  const win = pad * DAY;
  return EVENTS
    .filter((e) => e.country === code &&
      now >= +new Date(e.date + "T12:00:00") - win &&
      now <= +new Date(ends(e) + "T12:00:00") + win)
    .sort((a, b) => ends(b).localeCompare(ends(a)) || b.date.localeCompare(a.date));
}
...
export function searchAll(q) {
  ...
  for (const ev of sorted) {
    for (const en of getEntries(ev.id)) {
      ...
      /* owner / section match with no name hit — represent by the horse */
...
export function searchResultsHTML(q) {
  ...
  if (!r.events.length && !r.entries.length) {
    ...
    return '<div class="hs-empty">No matches — try a horse, rider, owner or event name</div>';
  }
  let html = "";
  if (r.events.length) {
