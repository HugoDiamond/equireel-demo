...
let archiveEntries = null, archivePromise = null;
const dataCbs = [];
export const entriesLoaded = () => !!realEntries;
export function onDataLoaded(cb) {
  /* listeners stay registered: entries.json and the 2019-22 archive arrive
     at different moments, and pages must re-render on each */
  dataCbs.push(cb);
  if (realEntries || archiveEntries) { try { cb(); } catch (e) {} }
  return () => { const i = dataCbs.indexOf(cb); if (i >= 0) dataCbs.splice(i, 1); };
}
const fireDataCbs = () => { dataCbs.slice().forEach((cb) => { try { cb(); } catch (e) {} }); };
export function loadRealEntries() {
  ...
  loadPromise = fetch(asset("/data/entries.json"))
    .then((r) => r.json())
    .then((j) => { realEntries = j; fireDataCbs(); })
    .catch(() => { loadPromise = null; });
  return loadPromise;
}

/* the 2019-2022 archive lives in its own file so the main payload stays
   fast — loaded lazily: idle-prefetched site-wide, and immediately on the
   pages that might need it (event/horse/rider). Fires the same listeners. */
export function loadArchiveEntries() {
  if (archivePromise || typeof window === "undefined") return archivePromise;
  archivePromise = fetch(asset("/data/entries-archive.json"))
    .then((r) => r.json())
    .then((j) => { archiveEntries = j; fireDataCbs(); })
    .catch(() => { archivePromise = null; });
  return archivePromise;
}

/* results-lite: finishing positions/statuses per (event, bib) from the
   platform DB — powers placing badges on career pages. Fires the same
   listeners so pages re-render when it arrives. */
let resultsLite = null, resultsLitePromise = null;
export function loadResultsLite() {
  if (resultsLitePromise || typeof window === "undefined") return resultsLitePromise;
  resultsLitePromise = fetch(asset("/data/results-lite.json"))
    .then((r) => r.json())
    .then((j) => { resultsLite = j; fireDataCbs(); })
    .catch(() => { resultsLitePromise = null; });
  return resultsLitePromise;
}
export function resultFor(eventId, bib) {
  if (!resultsLite || !resultsLite[eventId]) return null;
  const b = String(bib || "").replace(/\D/g, "") || String(bib || "");
  return resultsLite[eventId][b] || null;
}
...
export function getEntries(eventId) {
  ...
  if (REAL_IDS.has(eventId)) {
    const rows = (realEntries && realEntries[eventId]) ||
      (archiveEntries && archiveEntries[eventId]);
...
export const basket = {
  ...
  add(id) {
    if (id && !this.has(id)) {
      this._save([...this.ids(), id]);
      /* funnel signal — via window hook to avoid an import cycle */
      try { if (typeof window !== "undefined" && window.__eqTrack) window.__eqTrack("basket_add", { id }); } catch (e) {}
    }
  },
