"""Hybrid fill: for promos still on Ennis, add a SAME-EVENT FINISHER clip (any
class) whose length is closest to the promo's level (least-wrong class). Duration
filters out falls. Adds to promo_dropbox_map.json.

Run after option2_durations.py option3_probe.json. Then gen_promo_thumbs + gen_promos_v2.
"""
...
for evid, pairs in PROMOS.items():
    ...
    if not tgt:
        continue
    efiles = [r for r in event_files(evid, EVNAME.get(evid, evid)) if durations.get(r["href"])]
    if not efiles:
        continue
    ds = [durations[r["href"]] for r in efiles]
    med = statistics.median(ds)
    floor = max(120.0, 0.5 * med)
    finishers = [r for r in efiles if durations[r["href"]] >= floor]
    if not finishers:
        continue
...
print("hybrid same-event fills added: %d across %d events" % (fills, len(events)))
